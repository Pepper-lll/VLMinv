    ...
    def predict_answers(
        ...
        **kwargs
        ...
        assert inference_method in [
            ...
            "generate",
            "vqa_prob",
        ], "Inference method must be one of 'rank' or 'generate' or 'vqa_prob', got {}.".format(
            inference_method
        )
        ...
        if not inference_method == "vqa_prob":
        ...
        elif inference_method == 'vqa_prob':
            answer_prob = []
            probs = self._vqa_prob(samples, question_list)
            for p in probs:
                # answer = '{:.4f}'.format(p)
                # answer_prob.append(str(answer))
                answer_prob.append(p.item())
            return answer_prob

    def _vqa_prob(self, samples, question_list):
        question = self.tokenizer(question_list, padding='longest', truncation=True, max_length=35,
        ...
        question_atts = question.attention_mask
        ...
        prob_yes = prob[:, 2748] #token='yes'
        prob_no = prob[:, 2053] #token='no'
